...
This was the first vetting approach implemented in this pipeline. It helped, but not enough. Even with the full TCE battery applied, the false positive rate on a 250-target validation run was over 57%. The core problem was a calibration mismatch: NASA's TCE thresholds (SDE > 7.1, SNR floors, alias rejection criteria) were derived from their own internal pipeline: their detrending algorithm, their BLS implementation, their noise floor. Those numbers are not universal constants. They reflect the specific distribution of scores that NASA's system produces. This pipeline's biweight detrending and BLS configuration produce a different absolute score distribution for the same underlying signals, so noise artifacts that would score below NASA's thresholds in their system were scoring above them here. Long-period candidates were especially prone to this: with fewer observed transits in the baseline, the BLS power spectrum is noisier, and spurious peaks can score deceptively high in absolute terms. Compounding the problem, some of the checks (alias rejection, per-transit SNR floors) were simultaneously cutting real planets and admitting fake ones (the wrong shape of error in both directions). The TCE framework was implemented faithfully, tested, tuned, and ultimately found to be insufficient for the problem at hand.
